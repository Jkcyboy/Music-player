...
function togglePlayBack(action, index) {
    if (action === "play") {
        ...
        play_pause_btn.innerHTML = '<i class="fa fa-pause" aria-hidden="true"></i>'; // Fixed HTML
    } else if (action === "pause") {
        song[index].pause();
        song_status = false;
        clearInterval(update_second); // Clear interval
        wave_animation.style.opacity = '0'; // Fixed typo
...
function play_song() {
    ...
    current_singer_name.innerHTML = All_song[index_no].singer; // Fixed index issue

    setInterval(update_second, 1000); // Start updating song progress every second
    p_m_player.style.transform = 'translateY(0%)'; // Show popup player (Fixed typo: 'transfrom' → 'transform')
}
...
function update_second() {
    ...
    if (!isNaN(song[index_no].duration)) {
        position = song[index_no].currentTime * (100 / song[index_no].duration); // Fixed index issue
    ...
    total_duration.textContent = `${durationMinutes}:${durationSeconds < 10 ? '0' : ''}${durationSeconds}`; // Fixed variable name
...
//add functionallity to forward and back button
